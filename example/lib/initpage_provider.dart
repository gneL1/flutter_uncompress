...
class InitPageProvider with ChangeNotifier{
  ...
  Future onUncompress({String path}) async{
    ...
    await FlutterUncompress.uncompress(filePath: dir + '测试压缩.zip', uncompressPath: dir,progress: (value) async{
      if(_progress == value && value != 100)return;
      _text = '文件初始化 ： $value';
      _progress = value;
  ...
  Future onCopyFile() async{
    ...
    await FlutterUncompress.copyFileByBytes(bytes: bytes, path: dir,progress: (value) {
